...
PathFinder::PathFinder( Polygon_2&         outer_poly,
...
void PathFinder::addEdge(cPoint *current, Point_2 robotA, Point_2 robotB, bool robotAmoved) {
    cPoint *temp;
...
double PathFinder::cPointDistance(cPoint *a, cPoint *b) {
...
double PathFinder::heuristic(cPoint *cp) {
    return cPointDistance(cp, this->endCPoint);
}

void PathFinder::createArrangment() {
...
bool PathFinder::inLegalFace(const Point_2 &p) {
...
void PathFinder::setFaceRandomPoints(Face_handle face) {
...
void PathFinder::setRandomPoints() {
    Face_iterator it = arr.faces_begin();
    for(;it!=arr.faces_end();it++)
    {
        if(!it->contained())
...
void PathFinder::addVerticalSegment(Vertex_handle v, CGAL::Object obj, Kernel &ker) {
...
void PathFinder::verticalDecomposition(Kernel &ker) {
